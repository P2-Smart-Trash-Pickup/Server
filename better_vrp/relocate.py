...
def calculate_diff_relocate(
    ...
    cur_mass_1 = r1_weights[point_1]
    cur_mass_2 = r2_weights[point_1]
    ...
    old_1_pre_cost, old_1_pen_pre_cost, _ = calculate_segment_energy_points(
        point1=pre_1_val,
        point2=point_1,
    ...
    old_1_next_cost, old_1_pen_next_cost, _ = calculate_segment_energy_points(
        point1=point_1,
        point2=next_1_val,
        elevation_matrix=elevation_matrix,
        time_matrix=time_matrix,
    ...
    old_2_cost, old_2_pen_cost, old_2_time = calculate_segment_energy_points(
        point1=point_2,
        point2=next_2_val,
        elevation_matrix=elevation_matrix,
        time_matrix=time_matrix,
    ...
    new_1_cost, new_1_pen_cost, _ = calculate_segment_energy_points(
        point1=pre_1_val,
        point2=next_1_val,
        elevation_matrix=elevation_matrix,
        time_matrix=time_matrix,
    ...
    new_2_pre_cost, new_2_pre_pen_cost, new_2_pre_time = (
    ...
    new_2_next_cost, new_2_next_pen_cost, new_2_next_time = (
        calculate_segment_energy_points(
            point1=point_1,
            point2=next_2_val,
            elevation_matrix=elevation_matrix,
            ...
            penalty_matrix=r2_penalty_matrix,
            penalty=penalty,
            extra_mass=extra_2_mass + cur_mass_2,
        )
    )

    old_cost = old_1_pre_cost + old_1_next_cost + old_2_cost
    new_cost = new_2_pre_cost + new_2_next_cost + new_1_cost

    old_pen_cost = old_1_pen_pre_cost + old_1_pen_next_cost + old_2_pen_cost
    new_pen_cost = new_2_pre_pen_cost + new_2_next_pen_cost + new_1_pen_cost

    old_time = old_2_time
    new_time = new_2_pre_time + new_2_next_time + stop_delay

    reg_cost_diff = new_cost - old_cost
    pen_cost_diff = new_pen_cost - old_pen_cost
    time_diff = new_time - old_time

    return reg_cost_diff, pen_cost_diff, time_diff
...
def relocate_en(
    ...
    base_weights = sum_route_weight_volume(route2, r2_weights)
    base_volume = sum_route_weight_volume(route2, r2_volumes)
    base_time = sum_route_time(route2, time_matrix, stop_delay)
    best_i = 0
    best_j = 0

    for r_1_idx in range(1, len(route1) - 1):
        point_1 = route1[r_1_idx]
        point_weight = r2_weights[point_1]
        point_volume = r2_volumes[point_1]
        if base_weights + point_weight > r2_vehicle.max_capacity:
            continue

        base_power_consumption, _ = calculate_route_energy(
            route=route2,
            elevation_matrix=elevation_matrix,
            time_matrix=time_matrix,
            vehicle=r2_vehicle,
        ...
        for r_2_idx in range(len(route2) - 1):
            ...
            reg_cost_diff, pen_cost_diff, time_diff = calculate_diff_relocate(
                elevation_matrix=elevation_matrix,
                time_matrix=time_matrix,
                route1=route1,
                route2=route2,
            ...
            if base_power_consumption + reg_cost_diff > r2_vehicle.battery_capacity:
                continue
            if base_time + time_diff > r2_vehicle.time_constraint:
                continue
            if point_volume + base_volume > r2_vehicle.max_volume:
                continue
        ...
        for day_idx_j in range(len(day_plan)):
            ...
            for route_idx_i in range(len(cur_day_i)):
                ...
                for route_idx_j in range(len(cur_day_j)):
                    ...
                    temp_best_cost, temp_i, temp_j = relocate_en(
                        route1=route_i,
                        route2=route_j,
                        elevation_matrix=elevation_matrix,
                        time_matrix=time_matrix,
...
def relocate_cap(
    ...
    for r_1_idx in range(1, len(route1) - 1):
        point_1 = route1[r_1_idx]
        point_weight = r1_weights[point_1]
        if base_weights + point_weight > r2_vehicle.max_capacity:
            continue
        pre_1 = route1[r_1_idx - 1]
        next_1 = route1[r_1_idx + 1]

        old_1_cost = get_pen_rel(
            pre_1,
            point_1,
            next_1,
            distance_matrix=distance_matrix,
        ...
        for r_2_idx in range(len(route2) - 1):
            ...
            old_2_time = time_matrix[point_2][next_2]
            new_2_time = get_time_rel(point_2, point_1, next_2, time_matrix=time_matrix)
            time_diff = new_2_time - old_2_time

            pen_cost_diff = (new_2_cost + new_1_cost) - (old_2_cost + old_1_cost)

            if base_time + time_diff + stop_delay > r2_vehicle.time_constraint:
                continue
            if pen_cost_diff < best_improvement:
                best_improvement = pen_cost_diff
                best_i = r_1_idx
        ...
        for day_idx_j in range(len(day_plan)):
            ...
            for route_idx_i in range(len(cur_day_i)):
                ...
                for route_idx_j in range(len(cur_day_j)):
                    ...
                    temp_best_cost, temp_i, temp_j = relocate_cap(
                        route1=route_i,
                        route2=route_j,
                        distance_matrix=distance_matrix,
                        time_matrix=time_matrix,
...
def implement_relocate(
    ...
    route_1 = routes[route_1_idx]
    route_2 = routes[route_2_idx]
    point = route_1.pop(best_i)
    route_2.insert(best_j, point)
    routes[route_1_idx] = route_1
    routes[route_2_idx] = route_2


def implement_relocate_day(
    day_plan: list[list[list[int]]],
    best_i: int,
    best_j: int,
    best_route_i: int,
    ...
    day_1 = day_plan[best_day_i]
    day_2 = day_plan[best_day_j]

    route_1 = day_1[best_route_i]
    route_2 = day_2[best_route_j]

    point_i = route_1.pop(best_i)
    route_2.insert(best_j, point_i)

    day_1[best_route_i] = route_1
    day_2[best_route_j] = route_2
    ...
    if len(set(route_1)) - len(route_1) != -1:
        print("Route 1")
        print(point_i)
        print(route_1)
        raise Exception("Duplicate point")
    if len(set(route_2)) - len(route_2) != -1:
        print("Route 2")
        print(point_i)
        print(route_2)
        raise Exception("Duplicate point")
...
if __name__ == "__main__":
    ...
    best_imrp, best_i, best_j = relocate_cap(
        route1,
        route2,
        distance_matrix=distance_matrix,
        time_matrix=time_matrix,
    ...
    implement_relocate(routes, 0, 1, best_i, best_j)
    route1 = routes[0]
    route2 = routes[1]
    ...
    improvement,besti,bestj = relocate(route1=routes[0],route2=routes[1],time_matrix=time_matrix,elevation_matrix=elevation_matrix,weights=weights,penalty=0.0,penalty_matrix=penalty_matrix,vehicle=vehicle,volumes=fill_rates)
    print(besti)

    print(bestj)
    implement_relocate(routes,0,1,besti,bestj)
